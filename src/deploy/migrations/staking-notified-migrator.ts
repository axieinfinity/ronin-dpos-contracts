...
const deploy = async ({ getNamedAccounts, deployments, ethers }: HardhatRuntimeEnvironment) => {
  ...
  const proxyDepl = await deployments.get('StakingProxy');
  const newImplDelp = await deployments.get('StakingLogic');
  ...
  console.info('Deploying StakingNotifiedMigrator...');
  ...
  const deployment = await deploy('StakingNotifiedMigrator', {
...
deploy.dependencies = ['StakingProxy'];
