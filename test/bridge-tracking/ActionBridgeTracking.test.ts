...
let mockGatewayContract: MockGatewayForTracking;
...
describe('[Integration] Bridge Tracking test', () => {
  before(async () => {
    ...
    const mockGateway = await new MockGatewayForTracking__factory().deploy(bridgeTrackingAddress);
  ...
  it('Should be able to get contract configs correctly', async () => {
    expect(await bridgeTracking.bridgeContract()).eq(mockGatewayContract.address);
    expect(await mockGatewayContract.bridgeTrackingContract()).eq(bridgeTracking.address);
  ...
  //   await mockGatewayContract.connect(candidates[0].bridgeOperator).tryBulkDepositFor(receipts);
  //   await mockGatewayContract
  //     .connect(candidates[0].bridgeOperator)
  //     .tryBulkAcknowledgeMainchainWithdrew(mainchainWithdrewIds);
  //   await mockGatewayContract.connect(candidates[0].bridgeOperator).bulkSubmitWithdrawalSignatures(
  ...
  //   {
  //     const tx = await mockGatewayContract.connect(candidates[1].bridgeOperator).tryBulkDepositFor(receipts);
  //     await expect(tx).emit(mockGatewayContract, 'Deposited');
  //   }
  //   {
  //     const tx = await mockGatewayContract
  //       .connect(candidates[1].bridgeOperator)
  //       .tryBulkAcknowledgeMainchainWithdrew(mainchainWithdrewIds);
  //     await expect(tx).emit(mockGatewayContract, 'MainchainWithdrew');
  //   }
  //   await mockGatewayContract.connect(candidates[1].bridgeOperator).bulkSubmitWithdrawalSignatures(
  //     submitWithdrawalSignatures,
  //     submitWithdrawalSignatures.map(() => [])
  //   );
  // });
  ...
  //   await mockGatewayContract.connect(candidates[2].bridgeOperator).tryBulkDepositFor(receipts);
  //   await mockGatewayContract
  //     .connect(candidates[2].bridgeOperator)
  //     .tryBulkAcknowledgeMainchainWithdrew(mainchainWithdrewIds);
  //   await mockGatewayContract.connect(candidates[2].bridgeOperator).bulkSubmitWithdrawalSignatures(
  //     submitWithdrawalSignatures,
  //     submitWithdrawalSignatures.map(() => [])
  //   );
  ...
  //   await mockGatewayContract.connect(candidates[3].bridgeOperator).tryBulkDepositFor(receipts);
  //   await mockGatewayContract
  //     .connect(candidates[3].bridgeOperator)
  //     .tryBulkAcknowledgeMainchainWithdrew(mainchainWithdrewIds);
  //   await mockGatewayContract.connect(candidates[3].bridgeOperator).bulkSubmitWithdrawalSignatures(
  //     submitWithdrawalSignatures,
  //     submitWithdrawalSignatures.map(() => [])
  //   );
