...
abstract contract BOsGovernanceProposal is SignatureConsumer, IsolatedGovernance, IRoninGovernanceAdmin {
  ...
  /// @dev Mapping from period index => epoch index => voting extradata
  ...
  function _castVotesBySignatures(
    ...
    VotingSignature storage _extraData = _bridgeVoterSig[_ballot.period][_ballot.epoch];
    bool _hasValidVotes;
    ...
    for (uint256 _i = 0; _i < _signatures.length; _i++) {
      ...
      if (_weight > 0) {
        ...
        _extraData.signatureOf[_signer] = _signatures[_i];
        _extraData.voters.push(_signer);
