...
abstract contract ConditionalVersionControl is ERC1967Upgrade {
  ...
  /// @dev Error when this contract is interacted directly with call/staticcall
  error ErrOnlyDelegate(address addr);
  ...
  fallback() external payable virtual onlyDelegateFromProxyStorage {
    ...
    if (_isConditionMet()) {
      address(this).call(abi.encodeCall(this.tryUpgrade, ()));
    }
  ...
  function tryUpgrade() public onlyDelegateFromProxyStorage onlySelfCall {
